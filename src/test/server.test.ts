...
describe('Server', () => {
    ...
    describe('File upload', () => {
        ...
        it.skip('GCP Bucket', async () => {
            ...
            expect(Object.keys(body[0]).sort((a, b) => a.localeCompare(b))).toEqual(['fileId', 'originalFile'].sort((a, b) => a.localeCompare(b)))
            const { fileId } = body[0]
            // Check file contents
            const [uploadedFile, originalFile] = await Promise.all([
                got(`https://storage.googleapis.com/${options.bucket}${fileId as string}`, { encoding: undefined }),
